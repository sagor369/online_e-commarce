...
const CreateInvoice = () => {
  ...
  useEffect(() => {
    ...
    const hexNumber = timestamp.toString(16);
    setInvoiceNo(hexNumber);
  }, []);
    ...
    fetch("/api/invoice", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          toast({ description: "Inovice Created Successfully" });
  ...
  return (
    <div className="my-8 p-5 border shadow bg-white rounded-md">
      <h3 className="text-xl mb-5 text-center font-semibold">
        Create New Invoice
      </h3>
      <form onSubmit={handleSubmit(handleInvoice)}>
        ...
        <div className="flex items-start mt-3 justify-between gap-2">
          ...
          <div>
            ...
            <div className="text-right p-5">
              ...
              {(Number(totalAmount()) - Number(payAmount)).toFixed(2)}
            </div>
          </div>
        </div>
        <Button
        ...
        >
          {submitLoading ? <BtnSpinner /> : "Create Invoice"}
        </Button>
      </form>
    </div>
  );
